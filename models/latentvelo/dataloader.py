...
class Dataset(th.utils.data.Dataset):
    def __init__(self, adata, batch_size, shuffle=True, test=0.1, random_seed=42):
        ...
        # NOTE: modified by benchmark author
        n_batches = max(1, adata.shape[0] // batch_size)
        batch_indices = np.array_split(np.arange(adata.shape[0]), n_batches)

        # pre-determined batches, easier to use with adjacency matrix
        # for i in range(0, adata.shape[0], batch_size):
        for batch_idx in batch_indices: # NOTE: modified by benchmark author
            i = batch_idx[0]
            curr_batch_size = batch_idx.shape[0]

            # train and test index
            index_train, index_test = train_test_split(np.arange(adata.X[i:i+curr_batch_size,:].shape[0]), test_size=test, shuffle=False, random_state=i + 42)
            
            # select adajacency matrix
            adj_i = adj[i:i+curr_batch_size, i:i+curr_batch_size]
            self.adj.append(normalize(0.9*adj_i + scp.sparse.eye(adj_i.shape[0])))
            
            self.batches.append([i, i+curr_batch_size]) 
            self.index_train.append(index_train)
            self.index_test.append(index_test)
            index_test_ids.append(adata[i:i+curr_batch_size,:].obs.index[index_test])
