...
def preprocess(adata, n_bnn_neighbors=15, n_knn_neighbors=15, batch_mode="batch", batch_key="batch", batch_pair_list=None, sample_mode="random", is_ot=True, ratio_knn = None, ratio_bnn = None, filter_and_normalize=True, min_shared_counts=20, n_top_genes=2000, moments=True):
    if filter_and_normalize: # NOTE: modified by benchmark author
        scv.pp.filter_and_normalize(adata, min_shared_counts=min_shared_counts, n_top_genes=n_top_genes)
    if moments: # NOTE: modified by benchmark author
        if batch_mode == "batch":
            # 批次间单独建立邻居
            batch_pair_list = neighbor(adata, n_bnn_neighbors=n_bnn_neighbors, n_knn_neighbors=n_knn_neighbors, batch_key=batch_key, batch_pair_list=batch_pair_list, is_ot=is_ot, ratio_knn=ratio_knn, ratio_bnn=ratio_bnn)
        else:
