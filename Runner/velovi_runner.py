...
class veloVI_Runner(BaseRunner):
    def __init__(self, adata, pp_choice=0, n_hvg=2000, 
                 device = 0, 
                 save_dir = "logs/velovi", label=None):
        self.vae=None
        self.infered=False
        super().__init__(model_name=f"velovi", adata=adata, pp_choice=pp_choice, n_hvg=n_hvg, save_dir=save_dir, label=label, device=device)
        self.t_gene_key = "fit_t"
        self.tkey = None # velovi does not infer cellular latent time
    
    # def preprocess_real(self):
    #     scv.pp.filter_and_normalize(self.adata, min_shared_counts=20, n_top_genes=self.n_hvg) # FIXME: min_shared_counts changed from 30 to 20 on 2025-3-7
    ...
    def train(self):
        self.vae.train(accelerator="gpu")
