...
class VelocityMetric:
    ...
    def __str__(self):
        ...
        table.add_row(["velo_emb_na_ratio", round(self.spatial_velo_consist, 4)])
    ...
    def to_series(self):
        metric_dict = {"cbdir_mean": self.cbdir_mean,
                       ...
                       "velo_emb_na_ratio": self.velo_emb_na_ratio,
                       }
...
class BaseEvaluator:
    ...
    def evaluate(self, adata, runner:BaseRunner, vkey, tkey):
        self.compute_time_corr(adata, tkey=tkey)
        self.comput_spatial_time_consist(adata, tkey=tkey)
        adata_non_nan = adata[(adata.layers[f"{vkey}"] == 0).all(axis=1)].copy()
        self.metric.velo_emb_na_ratio = 1 - adata_non_nan.shape[0] / adata.shape[0]
